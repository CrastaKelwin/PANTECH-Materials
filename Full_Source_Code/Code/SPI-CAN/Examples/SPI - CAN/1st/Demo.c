/*
 * Project name:
     DEMO (CAN Network demonstration  CAN onboard module)
     1st Board demo
 * Description:
      The Can_1st node initiates the communication with the Can_2nd node by
      sending some data to its address. The 1st node then does the same and sends
      incremented data back to 2nd node, etc.
 * Test configuration:
     MCU:             LPC2148
     Dev.Board:       PRimer Board
     Oscillator:      12.0 MHz (cclk = 60.0 MHz, Fcco = 240.0 MHz)
     SW:              KEIL uVision3 v3.50
 * BOARD NOTES:
     - Place 100Kohm pull-up resistor networks RN1 to R9
     - Turn on CAN-CS, CAN-RST & CAN-INT located on DIP switch
     ...
     - The indicator of good communication in this example is,
       that the _1st node always displays data on PORTF P1[31..24],
...
int main (void)
{
	...
	//--- prepare port PORT1[23..16] as Initialization pgr indicator
	...
	CANSPISetFilter(CAN_FILTER_B2_F4,3,CAN_CONFIG_XTD_MSG);			// set id of filter B1_F1 to 3
	...
	CANSPIWrite (id,data,1,aa1);									// 1st node initiates communication


	while (1)
	{
		zr = CANSPIRead(&id , data , &len, &aa2);
		if ((id == 3) && zr)										// respond only to data from node No.3
		{
			...
			CANSPIWrite(id, data, 1,aa1);							// send incremented data back

			// if  message contains two data bytes...
//			if (len == 2)
//			{
//				IOCLR1 |= (0xFF << 16);									// output data at PORT1[23..16]
//				IOSET1 |= ((unsigned long)data[1] << 16) & 0x00FF0000;
//			}
